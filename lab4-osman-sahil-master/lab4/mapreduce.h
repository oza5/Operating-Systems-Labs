 ...
 * functions.
 */
typedef struct buffer_section{	//A struct that stores key-value pairs and points to the next buffer segment
	struct kvpair *kv;     //Helps us to iterate over the buffers and extract as well as store values depending on consumer-producer
	struct buffer_section* next;
}Buff_sec;
...
struct map_reduce {
	...
	map_fn mapfn;    
	reduce_fn reducefn;
	int buffer_size;  	 	//size of total buffer
	int *buff_count;	 	  	//tracks segments of buffer
	int *nmaps;	 	  	//number of mapper threads
	Buff_sec **buff_head;		//head of the circular buffer
	Buff_sec **buff_tail;		//tail of the circular buffer
	struct set_mapper_args *map_args; //mapper arguments
	int *c_jobs; 			//helps check for active/inactive threads in produce-consume
	pthread_mutex_t * thread_lock; //locks for avoiding race-conds
	pthread_t *buff_arr; 		//buffer threads
	struct set_reducer_args *red_args;  	//reducer arguments
};
